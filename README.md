...
- **Future Asset Managers**: Can easily inherit from StreamManager (e.g., FuturesStreamManager, ForexStreamManager)
...
- **StreamManager**: Generic streaming base class for any asset type (equities, futures, etc.)
...
# 2. Create futures stream manager (inherits from StreamManager)
class FuturesStreamManager(StreamManager):
    ...
    def add_futures_subscription(self, symbol: str) -> bool:
        if not self.futures_processor.validate_symbol(symbol):
            return False
        return self.add_subscription(symbol)

# 3. Create futures data manager
class FuturesDataManager:
    def __init__(self, data_dir):
        self.futures_stream_manager = FuturesStreamManager()
        # Futures-specific business logic here

# 4. Add to FeatureManager
def initialize_futures_data(self, schwab_client, schwab_streamer, is_mock_mode):
    # Initialize futures feature following same pattern as market data
    pass
```
