# options_flow.py - Modular Options Flow Monitor
import os
import time
import sqlite3
    ...
    def save_to_database(self, timestamp: int, call_delta_vol: float, put_delta_vol: float, 
        ...
        try:
            if self.db_connection_factory:
                ...
                # Create options flow table if it doesn't exist
                cursor.execute('''
                ...
                cursor.execute('''
                    ...
                    (symbol, timestamp, call_delta_vol, put_delta_vol, net_delta, 
                     delta_ratio, call_volume, put_volume, underlying_price, market_status, data_source)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (self.symbol, timestamp, call_delta_vol, put_delta_vol, net_delta,
                      ratio, call_vol, put_vol, underlying, market_status, 'SCHWAB_API'))
...
# Global instance - to be initialized by the main app
options_flow_monitor = None

def get_options_monitor() -> Optional[OptionsFlowMonitor]:
...
def initialize_options_monitor(symbol: str = "SPY", strike_count: int = 20, 
                             max_points: int = 100) -> OptionsFlowMonitor:
    """Initialize the global options flow monitor"""
    global options_flow_monitor
    options_flow_monitor = OptionsFlowMonitor(symbol, strike_count, max_points)
    return options_flow_monitor